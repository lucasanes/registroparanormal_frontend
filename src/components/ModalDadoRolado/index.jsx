...
export function ModalDadoRolado({ setModalClose, data }) {
  ...
  useEffect(() => {
    ...
    function rolarDado(valor) {
      if (data.isDano == false) {
        ...
        socket.emit('dado.rolado', {
          fichaId: id,
          nomeNPC: data.nomeNPC,
          nome: data.nome,
          isDano: data.isDano,
          isCritico: isCriticoA,
          isDesastre: isDesastreA,
    ...
    const valor = data.valor.replace(null, 0);

    rolarDado(dadoDinamico(valor, dc));
