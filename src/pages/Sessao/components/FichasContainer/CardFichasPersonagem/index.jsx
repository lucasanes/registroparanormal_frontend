...
export function CardFichasPersonagem({ data }) {
  ...
  async function handleEdit() {
    try {
      ...
      toast.success(
        `Esta ficha agora está ${!isPublic ? 'pública' : 'privada'}.`
      );
    } catch (e) {
      console.log(e);
    }
  ...
  return (
    <Container>
      <Header>
        ...
        <div>
          <LinkIcon
            to={`https://vdo.ninja/?view=${data.sessaoId}_sala_0`}
            target='_blank'
          >
      ...
      <Body>
        <TopBody>
          <Button
            onClick={() => setButtonActive('Principal')}
            active={buttonActive == 'Principal'}
          >
            Principal
          </Button>
          <Button
            onClick={() => setButtonActive('Status')}
            active={buttonActive == 'Status'}
          >
            Status
          </Button>
          <Button
            onClick={() => setButtonActive('Dados')}
            active={buttonActive == 'Dados'}
          >
            Dados
          </Button>
          <Button
            onClick={() => setButtonActive('Defesas')}
            active={buttonActive == 'Defesas'}
          >
            Defesas
          </Button>
        </TopBody>
        ...
        <BottomBody>
          {buttonActive == 'Principal' ? (
            <>
              <Grid>
                ...
                {(data.Principal[0].trilha != 'Nenhuma' ||
                  data.Principal[0].patente != 'Nenhuma') && (
                  <>
                    <Card>
                      <label>Trilha:</label>
            ...
            <>
              ...
              <h2>
                <b className='b'>Peso:</b> {pesoA}/{data.Status[0].peso}
              </h2>
            </>
          ) : buttonActive == 'Dados' ? (
            <>
              ...
              <DivDeferes>
                <Deferes>
                  <b>AGI</b>: {data.Atributos[0].agi}
                </Deferes>
                <Deferes>
                  <b>INT</b>: {data.Atributos[0].int}
                </Deferes>
                <Deferes>
                  <b>VIG</b>: {data.Atributos[0].vig}
                </Deferes>
                <Deferes>
                  <b>PRE</b>: {data.Atributos[0].pre}
                </Deferes>
                <Deferes>
                  <b>FOR</b>: {data.Atributos[0].for}
                </Deferes>
              </DivDeferes>
              ...
              <DivDeferes>
                {pericias.map((per) => (
                  <Deferes key={per.nome}>
                    <b>{periciasmap(per.nome)}</b>: {per.valor}
                  </Deferes>
                ))}
              </DivDeferes>
            </>
            ...
            buttonActive == 'Defesas' && (
              <>
                ...
                <DivDeferes>
                  {defesas.map((defesa) => (
                    <Deferes key={defesa.nome}>
                      <b>{defesa.nome}</b>: {defesa.valor}
                    </Deferes>
                  ))}
                </DivDeferes>
                ...
                <DivDeferes>
                  {res.map((resist) => (
                    <Deferes key={resist.nome}>
                      <b>{resistenciasmap(resist.nome)}</b>: {resist.valor}
                    </Deferes>
                  ))}
                </DivDeferes>
              </>
