...
export function Sessao({ data, sessoes, setSessoes }) {
  ...
  useEffect(() => {
    ...
    if (data.Participantes) {
      const cadaParticipante = data.Participantes?.map(
        (each) => each.user.nome
      );
  ...
  async function handleDelete() {
    try {
      ...
      const sessoesAtualizadas = sessoes.filter(
        (sessao) => sessao.id != data.id
      );
  ...
  return (
    <Container>
      <Modal
        isOpen={modalEditarSessaoIsOpen}
        setClose={() => setModalEditarSessaoIsOpen(false)}
      >
        <ModalEditSessao
          data={data}
          sessoes={sessoes}
          setSessoes={setSessoes}
          setModalClose={() => setModalEditarSessaoIsOpen(false)}
        />
      </Modal>
      ...
      <Header>
        <h2>{data.nome}</h2>
        <div>
          <Link to={`/streaming/${data.id}`} target='_blank'>
            <MdOutlinePlayCircle size={20} />
          </Link>
          <Link
            to={`https://vdo.ninja/?push=${data.id}_sala_10`}
            target='_blank'
          >
            <BsCameraVideo size={20} />
          </Link>
          <Link to={`/sessao/documentos/${data.id}`} target='_blank'>
            <IoDocumentsOutline size={20} />
          </Link>
          <Button
            color={'blue'}
            onClick={() => setModalEditarSessaoIsOpen(true)}
          >
            <BsGear size={18} />
          </Button>
